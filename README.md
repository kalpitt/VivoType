...
### Install a release (fastest — no build required)

**Homebrew:**
```bash
brew install --cask kalpitt/vivotype/vivotype
```
...
Both download the latest signed `VivoType.app` release and install it to
`/Applications`. Requires an Apple Silicon Mac (M1/M2/M3/M4) running macOS.

**Launch and dictate** — open VivoType from `/Applications`. On first launch a
**“Welcome to VivoType”** window sets up its private engine (needs
**Python 3.11+**; if it's missing, VivoType offers a **Retry** button after you
install it), then a guided **Permissions** checklist walks you through granting
Microphone and Accessibility access. Once granted, hold **`Right-Option`**,
speak, release — your words appear in the focused app. See
[clients/mac/README.md](clients/mac/README.md) for details.

> VivoType is fully self-contained. To remove it: `brew uninstall --cask
> vivotype` (Homebrew) or drag `VivoType.app` to the Trash (manual install),
> then delete `~/Library/Application Support/VivoType/`.
...
and follow the same first-launch setup and Permissions checklist described above.
